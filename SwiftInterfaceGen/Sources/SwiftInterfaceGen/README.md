...
| **`GenerateStubs.swift`** | `extension SwiftInterfaceGen { static func generateStubs(...) }` — synthesizes one dependency-stub `.swift` file per referenced private framework so a target's generated interface can import module-qualified types from frameworks that aren't themselves being rebuilt. Purely an organizational split out of `main.swift` — no behavior change. |
