...
| CoreAICompiler | 0 | 0 |
| CoreAICommon | 0 | 0 |
| ModelCatalog | 11 (`fA_` default-arg accessors + 1 constrained ext) | **0** |
...
### Why assembly stubs are still needed
...
|---|---|---|
| `fA_` default-arg accessors | `Catalog.isIFPEnabled(gestalt:)fA_` | Require constant-expression defaults; `dummyDefaultValue()` doesn't produce an exported data symbol |
| Constrained extension methods | `ResourceBundleIdentifier<where A==LLMBundle>.serverConfiguration()` | Complex type-equality constraints not yet parsed |
...
`fA_` / `fA0_` … symbols are pre-mapped to their parent function in `processSymbols()`. `Model.swift` injects `= dummyDefaultValue()` (or a typed constant for simple types) at the correct parameter positions, enabling the Swift compiler to emit native default-argument accessor symbols where possible.
...
Reads the expected symbol list from `<exports_file>`, inspects the compiled dylib with `nm -gU`, and emits assembly stubs (`.quad 0` data entries) for every missing symbol into `<stubs_output.s>`. Also reports extra symbols.
