...
Applying this to every remaining category above (all confirmed unfixable at the *declaration* level) took the curated-19 total from ~1569 first-pass stubs to **0**, stable across repeated `verify_public.py` runs. There's a `selfAlignInterface` function already in `main.swift` (search for it) that sketches a fully automatic version of this idea — compile the generated interface once, diff against the TBD, forge whatever's still missing — but it's unwired dead code (hardcoded `/tmp` paths, no `-import-objc-header`, no bridge-object linking, never called from anywhere) and wasn't safe to flip on blind; the per-framework blocks added at the end of `postProcess()` were built by hand against fresh `stubs_<Framework>.s` listings instead.
