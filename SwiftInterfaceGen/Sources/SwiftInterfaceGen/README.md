...
Public-framework ground-truth suite (`verify_public.py`), 19 curated SDK frameworks — 19/19 PASS, 0 missing symbols (compiles cleanly against real SDK `.swiftinterface`/`.tbd`, independent of the regression suite above). Total first-pass stubs across all 19 sits at ~1569, down from a several-thousand baseline across many sessions of root-causing individual gaps (HealthKit, SoundAnalysis, Combine, Charts, CoreML, Network, and others — see git history on `main.swift` for the fix-by-fix record). The remaining stubs are not believed to be further reducible without new generator features; they cluster into a handful of confirmed structural walls — see [Confirmed-unfixable stub categories](#confirmed-unfixable-stub-categories) below.
...
Across a long series of sessions eliminating first-pass stubs from the curated 19 public frameworks (`verify_public.py`), the remaining ~1569 stubs were traced to a small number of structural walls, each confirmed via minimal standalone `swiftc` repros compiled with the same flags `verify_public.py` uses (`-enable-library-evolution -language-mode 6 -enable-experimental-feature NonescapableTypes -enable-experimental-feature Lifetimes`). None of these are fixable by changing the *generated Swift source* — they require either a different host toolchain or new generator features (parameter-pack support). Recognizing them quickly avoids re-investigating the same dead end framework after framework:
