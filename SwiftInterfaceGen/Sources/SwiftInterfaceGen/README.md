...
| **`SymbolCompare.swift`** | `extension SwiftInterfaceGen { static func runCompare(...) }` (the `--compare` CLI mode) plus its shared helpers `extractDylibSymbols`, `filteredExportSymbols`, `extractMissingModules`, and `emitEmptyStubFramework` — diffs a compiled dylib's real exported symbols against the expected `.tbd` set and emits `@_silgen_name` assembly stubs for anything missing. Also used by `SelfAlign.swift` and `main()`'s own exports-file writer. Purely an organizational split out of `main.swift` — no behavior change. |
