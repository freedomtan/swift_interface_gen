...
- [x] **Public-framework ground-truth verification (`verify_public.py`), curated 18-framework suite: 11/18 PASS** (see `verify_public_plan.md` for per-framework detail and the remaining 7).
  - Fixed: NearbyInteraction, Translation, TipKit, CoreML, MetricKit, TabularData, SwiftData, CryptoKit, Combine, StoreKit, SoundAnalysis.
  - Each fix verified against zero regressions in the 9 private targets (`run_regression_tests.py`) plus Combine (`orchestrate.py`).
  - Remaining: Charts, Speech, CreateML, GameKit, HealthKit, Vision, Network.
