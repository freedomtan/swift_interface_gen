...
**Integration Testing & AMFI:** 
For integration tests that interact with system services (like `test_ModelCatalog.swift`), codesigning with private entitlements is required:
```bash
codesign --force --options runtime --entitlements ModelCatalog.entitlements -s - test_integration_run
```
**Note:** This ad-hoc codesigning is generally only effective for execution on devices where **AMFI (Apple Mobile File Integrity)** has been disabled. Basic metadata tests (like `test_types_run`) do not require entitlements or codesigning to run.
